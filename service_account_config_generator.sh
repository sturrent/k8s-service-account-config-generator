...
SA_TOKEN="$(kubectl -n "$NAMESPACE" create token "$SA_NAME" --duration 8h)"
...
ENDPOINT="$(kubectl config view --raw --minify --flatten  --output 'jsonpath={.clusters[].cluster.server}')"

# get ca cert
kubectl config view --raw --minify --flatten  --output 'jsonpath={.clusters[].cluster.certificate-authority-data}' | base64 -d > /tmp/temp_ca_server.crt
